...
export class OpenAIService {
  ...
  async getChatResponse(message: string, context: string = ''): Promise<string> {
    ...
    try {
      const systemPrompt = `You are a friendly and culturally knowledgeable temple tour guide from Kumbakonam, Tamil Nadu.
...
Respond like you're talking to a tourist who's new to Kumbakonam.`;
  ...
  private getFallbackResponse(message: string): string {
    ...
    if (lowerMessage.includes('temple') && lowerMessage.includes('timing')) {
      return "🕉️ Most temples in Kumbakonam are open from 6:00 AM to 12:30 PM and 4:00 PM to 9:00 PM daily. The morning hours are especially peaceful for prayers and darshan.";
    }
    
    if (lowerMessage.includes('mahamaham')) {
      return "🎊 Mahamaham is the grand festival held once every 12 years in Kumbakonam! During this time, millions of devotees gather at the Mahamaham tank for a holy dip. The next celebration will be truly spectacular.";
    }
    
    if (lowerMessage.includes('hotel') || lowerMessage.includes('stay')) {
      return "🏨 Kumbakonam has several good accommodation options ranging from budget stays to premium hotels. You can check our Hotels section for detailed information about stays near the temples.";
    }
    ...
    return "🙏 Thank you for your question! I'm here to help you explore the beautiful temples and heritage of Kumbakonam. Could you be more specific about what you'd like to know?";
  }
}
